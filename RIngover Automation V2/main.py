...
class CSVDialerApp(ctk.CTk):
    ...
    def build_ui(self):
        ...
        self.column_menu = ctk.CTkOptionMenu(self.frame_top, values=["Colonne Tél"], command=self.on_column_selected)
    ...
    def set_tag(self, tag_name):
        if self.is_running or self.current_index < len(self.csv_data):
            ...
            self.update_console(f"Tag mis à jour : {tag_name}")
    ...
    def load_csv(self):
        ...
        try:
            ...
            self.update_console(f"CSV chargé : {len(self.csv_data)} lignes.")
            self.current_index = 0
    ...
    def start_calib(self, mode):
        ...
        if not hwnd: 
            messagebox.showerror("Erreur", "Ringover non trouvé")
            return
    ...
    def toggle_campaign(self):
        if not self.is_running:
            ...
            self.btn_start.configure(text="ARRÊTER / PASSER SUIVANT", fg_color="#c0392b")
    ...
    def campaign_loop(self):
        while self.is_running and self.current_index < len(self.csv_data):
            ...
            if hwnd and phone:
                ...
                # Petit délai pour laisser le temps de mettre un tag final
    ...
    def finish_campaign(self):
        self.btn_start.configure(text="▶ CAMPAGNE TERMINÉE", fg_color="#27ae60")
        self.btn_download.configure(state="normal")
        messagebox.showinfo("Fini", "Tous les contacts ont été traités.")
    ...
    def wait_for_call_end(self, hwnd):
        # Détection du bouton rouge de Ringover
        is_calling = False
        for _ in range(20):
